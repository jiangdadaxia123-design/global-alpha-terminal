...
def get_yfinance_data(symbol, interval):
    try:
        # 映射周期：支持 4h
        if "4小时" in interval: yf_interval = "1h" # Yahoo暂用1h替代
...
def get_market_data(asset_type, symbol, interval, use_proxy_setting, proxy_url_setting):
    ...
    if is_cn_stock and "4小时" in interval:
        st.toast("⚠️ A股数据源暂不支持分时数据，自动切换为【日线】。", icon="🔄")
        interval = "日线 (1D)" # 强制降级
...
def generate_outlook(data, interval_label):
    
    if "4小时" in interval_label:
        safe_threshold, risk_threshold = 1.03, 1.15
        period_desc = "4H短线"
    elif "日线" in interval_label:
        safe_threshold, risk_threshold = 1.05, 1.30
        period_desc = "日线"
    else: 
        safe_threshold, risk_threshold = 1.15, 1.45
        period_desc = "中长线"

    if data['ratio'] < safe_threshold:
        sell_status, sell_desc, sell_score = "🟢 极低抛压", f"回踩{period_desc}成本线，获利盘清洗完毕。", 1
    elif data['ratio'] < risk_threshold:
        sell_status, sell_desc, sell_score = "🟡 正常换手", "趋势延续中。", 0
    else:
        sell_status, sell_desc, sell_score = "🔴 高位获利", f"乖离率过大 (>{risk_threshold})，注意回调。", -1
...
with st.sidebar:
    ...
    if asset_class == "Crypto (币安)":
        ...
        symbol_map = {
            ...
            "【宽基】沪深300ETF (510300)": "510300",
            "【宽基】科创50ETF (588000)": "588000",
            "【宽基】创业板ETF (159915)": "159915",
            "【ETF】红利ETF (510880)": "510880",
            "【ETF】煤炭ETF (515220)": "515220",
            "【核心】贵州茅台": "600519", 
            "【核心】宁德时代": "300750", 
            "【核心】东方财富": "300059",
    ...
    interval_ui = st.radio("3. 分析周期", ["4小时 (4H) - 短线", "日线 (1D)", "周线 (1W)", "月线 (1M)"])
...
if df_raw is not None:
    ...
    if data:
        logic = generate_outlook(data, interval_ui)
        ...
        with col1:
            ...
            c2.metric("成本偏离度", f"{data['ratio']:.2f}", delta="< 安全阈值", delta_color="inverse")
            ...
            fig_lth.update_layout(
                ...
                font={'color':'#ccc'},
                xaxis=dict(showgrid=False, showticklabels=True, title="日期"), 
                yaxis=dict(gridcolor='#333', showticklabels=True), 
                showlegend=True,
        ...
        with col2:
            ...
            fig_vol.update_layout(
                ...
                font={'color':'#ccc'}, 
                xaxis=dict(showgrid=False, showticklabels=True, title="日期"), 
                yaxis=dict(gridcolor='#333', showticklabels=True, title="成交量"), 
                showlegend=False
            )
